...
import cv2
import neuralgym as ng ; import numpy as np
...
class InpaintCAModel(Model):
    ...
    def build_inpaint_net(self, x, mask, reuse=False, FLAGS=None,
        ...
        with tf.variable_scope(name, reuse=reuse), \
            ...
            # import time; c1_stime = time.time()# stage1
            x = gen_conv(x, cnum, 5, 1, name='conv1')#; c1_etime = time.time(); c1_intv = c1_etime-c1_stime; print('\n\n\n{:*^50}\n\n\n'.format('interval info - time')); print('c1 interval time: {}\n'.format(c1_intv)); print('\n\n\n{:*^50}\n\n\n'.format('-')); 
            ...
            x, offset_flow = contextual_attention(x, x, mask_s, 3, 1, rate=2, FLAGS=FLAGS)
            x = gen_conv(x, 4*cnum, 3, 1, name='pmconv9')
            x = gen_conv(x, 4*cnum, 3, 1, name='pmconv10')
            pm = x 
            x = tf.concat([x_hallu, pm], axis=3)
            # of_shape = offset_flow.get_shape().as_list()
            # if of_shape[3] > 1: 
            # if FLAGS.img_shapes[2] == 1:
            #     offset_flow = tf.reshape(offset_flow[:, :, :, 1], [of_shape[0], of_shape[1], of_shape[2], 1])
    ...
    def build_graph_with_losses(
        ...
        if FLAGS.MIT_mask:
            ...
            mask = np.load('/home/yangpan/projs/MITTEC_mask/MITTEC_mask_{}.npy'.format(rand_int)).reshape(1, 64, 64, 1).astype(np.float32) 
            #mask = np.load('/home/yangpan/projs/MITTEC_mask.npy').reshape(1, 64, 64, 1).astype(np.float32)
        ...
        x1, x2, offset_flow = self.build_inpaint_net(
            xin, mask, reuse=reuse, training=training, FLAGS=FLAGS,
            padding=FLAGS.padding)
        batch_predicted = x2 #[:, :, :, 0]; batch_predicted = tf.reshape(batch_predicted, [FLAGS.batch_size, 64, 64, 1])
        losses = {} #; offset_flow = None#offset_flow = tf.reshape(offset_flow[:, :, :, 0], [FLAGS.batch_size, 16, 16, 1])
    ...
    def build_infer_graph(self, FLAGS, batch_data, bbox=None, name='val'):
        ...
        # inpaint
        x1, x2, offset_flow = self.build_inpaint_net(
            xin, mask, reuse=True, FLAGS=FLAGS,
    ...
    def build_server_graph(self, FLAGS, batch_data, reuse=False, is_training=False):
        ...
        x1, x2, flow = self.build_inpaint_net(
            xin, masks, reuse=reuse, FLAGS=FLAGS, training=is_training)
        batch_predict = x2
        # apply mask and reconstruct
